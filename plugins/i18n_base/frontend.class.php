  ...
  public static function outputHeader($full=true) {
    ...
    if ($metad != '') {
      ...
      if (function_exists('mb_substr')) { 
        $description = trim(mb_substr(html_entity_decode(strip_tags(stripslashes(htmlspecialchars_decode($content, ENT_QUOTES))),ENT_QUOTES), 0, 160));
      } else {
        $description = trim(substr(html_entity_decode(strip_tags(stripslashes(htmlspecialchars_decode($content, ENT_QUOTES))),ENT_QUOTES), 0, 160));
      }
